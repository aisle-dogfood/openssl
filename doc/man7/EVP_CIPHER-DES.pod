...
B<WARNING:> DES3-WRAP uses SHA-1 for integrity checking, which is cryptographically
weak and not FIPS-approved. This algorithm is provided only for legacy compatibility
and should not be used in new systems. Use modern key wrapping algorithms such as
AES-KW (RFC 3394) or AES-KWP (RFC 5649) instead.
