...
B<Security Note:> This trace category should only be enabled in controlled
debugging environments. It logs non-sensitive metadata (iteration count and
data lengths) but does not include passwords, salts, or derived keys to
prevent accidental leakage in production systems.
...
B<Security Note:> This trace category should only be enabled in controlled
debugging environments. It logs non-sensitive metadata (ID, iteration count,
and data lengths) but does not include passwords, salts, or derived keys to
prevent accidental leakage in production systems.
