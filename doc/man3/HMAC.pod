...
If I<md> is NULL, the digest is placed in a static array protected by a mutex
for thread safety. The size of the output is placed in I<md_len>, unless it
is NULL. Note: while passing NULL for I<md> is now thread-safe, it may cause
performance degradation in multi-threaded applications due to mutex contention.
For best performance in multi-threaded environments, callers should provide
