...
If I<md> is NULL, the digest is placed in a thread-local buffer.  The size of
the output is placed in I<md_len>, unless it is NULL.
Note: When I<md> is NULL, the returned pointer points to thread-local storage
that persists for the lifetime of the thread. For better control over memory
management, callers should provide their own buffer by passing a non-NULL I<md>.
