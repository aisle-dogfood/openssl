...
Specifies the connection timeout in seconds. This is the maximum time allowed
for the connection to the timestamp server to be established. If the connection
cannot be established within this time, the request will fail. Default is 60
seconds. (Optional)

=item B<-T> I<seconds>

Specifies the total timeout in seconds. This is the maximum time allowed for
the entire timestamp request operation, including connection establishment,
sending the request, and receiving the response. If the operation is not
completed within this time, the request will fail. Default is 300 seconds.
(Optional)
