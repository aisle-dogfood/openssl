...
=item B<-T> I<connect_timeout>

Maximum time in seconds allowed for the connection to the server to be
established. A value of zero disables the timeout (not recommended). If not
specified, a default timeout of 30 seconds is used. (Optional)

=item B<-t> I<timeout>

Maximum time in seconds allowed for the entire request/response operation. This
includes connection establishment, sending the request, and receiving the
response. A value of zero disables the timeout (not recommended). If not
specified, a default timeout of 120 seconds is used. (Optional)
