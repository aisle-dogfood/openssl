...
/*
 * Maximum buffer size to prevent unbounded memory allocation.
 * 64 MB should be sufficient for legitimate use cases while preventing DoS.
 */
...
static int readbuffer_resize(BIO_F_BUFFER_CTX *ctx, int sz)
{
    ...
    /* Enforce maximum buffer size to prevent unbounded memory allocation */
