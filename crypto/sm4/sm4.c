 ...
 * Performs a lookup in the S-box without secret-dependent memory accesses
 */
static ossl_inline uint8_t sm4_sbox_lookup_ct(uint8_t index)
{
    ...
    for (i = 0; i < 256; i++) {
        uint8_t mask = constant_time_eq_8(i, index);
        result |= SM4_S[i] & mask;
    }
...
void ossl_sm4_encrypt(const uint8_t *in, uint8_t *out, const SM4_KEY *ks)
{
    ...
    /*
     * Uses constant-time byte-wise S-box for all rounds to prevent
