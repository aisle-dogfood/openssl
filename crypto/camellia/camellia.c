...
/* Byte-wise S-box for cache-timing resistance */
static const u8 Camellia_SBOX_BYTE[256] = {
...
/*
 * Cache-timing resistant version using byte-wise S-box lookups.
 * This is slower but provides some protection from cache-based side channels.
 * Used in first and last rounds to reduce timing attack surface.
 * 
 * This implements the same logic as Camellia_Feistel but uses only byte lookups
 * to reduce cache timing leakage from secret-indexed table accesses.
 */
#define Camellia_Feistel_CT(_s0,_s1,_s2,_s3,_key) do {\
        register u32 _t0,_t1,_t2,_t3;\
        u8 _x;\
\
        _t0  = _s0 ^ (_key)[0];\
        _t1  = _s1 ^ (_key)[1];\
\
        /* Process t0 through byte S-box with required permutations */\
        _x = Camellia_SBOX_BYTE[(_t0 >> 0) & 0xff];\
        _t3  = ((u32)_x) | ((u32)_x << 8) | ((u32)_x << 16) | ((u32)_x << 24);\
\
        _x = Camellia_SBOX_BYTE[(_t0 >> 8) & 0xff];\
        _t3 ^= ((u32)_x << 24) | ((u32)_x) | ((u32)_x << 8) | ((u32)_x << 16);\
\
        _x = Camellia_SBOX_BYTE[(_t0 >> 16) & 0xff];\
        _t3 ^= ((u32)_x) | ((u32)_x << 16) | ((u32)_x << 8) | ((u32)_x << 24);\
\
        _x = Camellia_SBOX_BYTE[(_t0 >> 24) & 0xff];\
        _t3 ^= ((u32)_x << 8) | ((u32)_x << 16) | ((u32)_x << 24) | ((u32)_x);\
\
        /* Process t1 through byte S-box with required permutations */\
        _x = Camellia_SBOX_BYTE[(_t1 >> 0) & 0xff];\
        _t2  = ((u32)_x << 8) | ((u32)_x << 16) | ((u32)_x << 24) | ((u32)_x);\
\
        _x = Camellia_SBOX_BYTE[(_t1 >> 8) & 0xff];\
        _t2 ^= ((u32)_x) | ((u32)_x << 8) | ((u32)_x << 16) | ((u32)_x << 24);\
\
        _x = Camellia_SBOX_BYTE[(_t1 >> 16) & 0xff];\
        _t2 ^= ((u32)_x << 24) | ((u32)_x) | ((u32)_x << 8) | ((u32)_x << 16);\
\
        _x = Camellia_SBOX_BYTE[(_t1 >> 24) & 0xff];\
        _t2 ^= ((u32)_x) | ((u32)_x << 16) | ((u32)_x << 8) | ((u32)_x << 24);\
\
        /* Combine results same as standard Feistel */\
        _t2 ^= _t3;\
        _t3  = RightRotate(_t3,8);\
        _s3 ^= _t3;\
        _s2 ^= _t2; \
                                  ...
                                  u8 ciphertext[])
{
    ...
    while (1) {
        ...
        /* Use cache-timing resistant version for first round */
        if (k == keyTable + 4) {
            Camellia_Feistel_CT(s0, s1, s2, s3, k + 0);
            Camellia_Feistel_CT(s2, s3, s0, s1, k + 2);
        } else {
        ...
        /* Use cache-timing resistant version for last round */
        if (k + 12 == kend) {
            Camellia_Feistel_CT(s0, s1, s2, s3, k + 8);
            Camellia_Feistel_CT(s2, s3, s0, s1, k + 10);
        } else {
                                  ...
                                  u8 plaintext[])
{
    ...
    while (1) {
        /* Camellia makes 6 Feistel rounds */
        k -= 12;
        /* Use cache-timing resistant version for first round of decryption */
        if (k == keyTable + grandRounds * 16 - 12) {
            Camellia_Feistel_CT(s0, s1, s2, s3, k + 10);
            Camellia_Feistel_CT(s2, s3, s0, s1, k + 8);
        } else {
        ...
        /* Use cache-timing resistant version for last round of decryption */
        if (k == kend) {
            Camellia_Feistel_CT(s0, s1, s2, s3, k + 2);
            Camellia_Feistel_CT(s2, s3, s0, s1, k + 0);
        } else {
