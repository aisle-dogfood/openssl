                                   ...
                                   const EVP_MD *digest)
{
    ...
    /* Validate salt length to prevent weak salt vulnerability (SP800-132) */
    if (saltlen < 16) {  /* Minimum 128 bits (16 bytes) as per SP800-132 */
                              ...
                              const X509_ALGOR *macalg, unsigned char *key)
{
    ...
    /* Validate salt length to prevent weak salt vulnerability (SP800-132) */
    if (pbkdf2_salt->length < 16) {  /* Minimum 128 bits (16 bytes) as per SP800-132 */
        ERR_raise(ERR_LIB_PKCS12, PKCS12_R_MAC_SETUP_ERROR);
        goto err;
    }
