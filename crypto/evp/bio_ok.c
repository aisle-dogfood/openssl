...
static int sig_out(BIO *b)
{
    ...
    if (!EVP_DigestInit_ex(md, digest, NULL))
        goto berr;
    /*
     * Generate random seed and use it to initialize the digest.
     * This replaces the previous fragile approach of writing directly
     * into EVP_MD_CTX internals, which is incompatible with provider-backed digests.
     */
    ...
    /* Use the seed as input to the digest */
...
static int sig_in(BIO *b)
{
    ...
    if (!EVP_DigestInit_ex(md, digest, NULL))
        goto berr;
    /*
     * Read the seed from the buffer and use it to initialize the digest.
     * This replaces the previous fragile approach of writing directly
     * into EVP_MD_CTX internals, which is incompatible with provider-backed digests.
     */
