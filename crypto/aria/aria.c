...
/*
 * Constant-time table lookup to mitigate cache-timing attacks.
 * Scans entire table and uses constant_time_select to pick the value
 * without secret-dependent memory access patterns.
 */
static ossl_inline uint32_t aria_ct_lookup_u32(const uint32_t *table,
                                               uint8_t index)
{
    uint32_t result = 0;
    unsigned int i;
    ...
    for (i = 0; i < 256; i++) {
        uint32_t mask = constant_time_eq_8((uint8_t)i, index);
        mask |= mask << 8;
        mask |= mask << 16;
        result |= table[i] & mask;
    }
...
static ossl_inline uint8_t aria_ct_lookup_u8(const uint8_t *table,
                                             uint8_t index)
{
    uint8_t result = 0;
    unsigned int i;
    ...
    for (i = 0; i < 256; i++) {
        uint8_t mask = constant_time_eq_8((uint8_t)i, index);
        result |= table[i] & mask;
    }
    return result;
}
...
/* S-Box Layer 1 + M (constant-time to mitigate cache attacks) */
#define ARIA_SBOX_LAYER1_WITH_PRE_DIFF(T0, T1, T2, T3)  \
    do {                                                \
        (T0) =                                          \
            aria_ct_lookup_u32(S1, GET_U8_BE(T0, 0)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T0, 1)) ^  \
            aria_ct_lookup_u32(X1, GET_U8_BE(T0, 2)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T0, 3));   \
        (T1) =                                          \
            aria_ct_lookup_u32(S1, GET_U8_BE(T1, 0)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T1, 1)) ^  \
            aria_ct_lookup_u32(X1, GET_U8_BE(T1, 2)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T1, 3));   \
        (T2) =                                          \
            aria_ct_lookup_u32(S1, GET_U8_BE(T2, 0)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T2, 1)) ^  \
            aria_ct_lookup_u32(X1, GET_U8_BE(T2, 2)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T2, 3));   \
        (T3) =                                          \
            aria_ct_lookup_u32(S1, GET_U8_BE(T3, 0)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T3, 1)) ^  \
            aria_ct_lookup_u32(X1, GET_U8_BE(T3, 2)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T3, 3));   \
    } while(0)

/* S-Box Layer 2 + M (constant-time to mitigate cache attacks) */
#define ARIA_SBOX_LAYER2_WITH_PRE_DIFF(T0, T1, T2, T3)  \
    do {                                                \
        (T0) =                                          \
            aria_ct_lookup_u32(X1, GET_U8_BE(T0, 0)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T0, 1)) ^  \
            aria_ct_lookup_u32(S1, GET_U8_BE(T0, 2)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T0, 3));   \
        (T1) =                                          \
            aria_ct_lookup_u32(X1, GET_U8_BE(T1, 0)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T1, 1)) ^  \
            aria_ct_lookup_u32(S1, GET_U8_BE(T1, 2)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T1, 3));   \
        (T2) =                                          \
            aria_ct_lookup_u32(X1, GET_U8_BE(T2, 0)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T2, 1)) ^  \
            aria_ct_lookup_u32(S1, GET_U8_BE(T2, 2)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T2, 3));   \
        (T3) =                                          \
            aria_ct_lookup_u32(X1, GET_U8_BE(T3, 0)) ^  \
            aria_ct_lookup_u32(X2, GET_U8_BE(T3, 1)) ^  \
            aria_ct_lookup_u32(S1, GET_U8_BE(T3, 2)) ^  \
            aria_ct_lookup_u32(S2, GET_U8_BE(T3, 3));   \
    } while(0)
                       ...
                       const ARIA_KEY *key)
{
    ...
    reg0 = rk->u[0] ^ MAKE_U32(
        (uint8_t)(aria_ct_lookup_u32(X1, GET_U8_BE(reg0, 0))     ),
        (uint8_t)(aria_ct_lookup_u32(X2, GET_U8_BE(reg0, 1)) >> 8),
        (uint8_t)(aria_ct_lookup_u32(S1, GET_U8_BE(reg0, 2))     ),
        (uint8_t)(aria_ct_lookup_u32(S2, GET_U8_BE(reg0, 3))     ));
    reg1 = rk->u[1] ^ MAKE_U32(
        (uint8_t)(aria_ct_lookup_u32(X1, GET_U8_BE(reg1, 0))     ),
        (uint8_t)(aria_ct_lookup_u32(X2, GET_U8_BE(reg1, 1)) >> 8),
        (uint8_t)(aria_ct_lookup_u32(S1, GET_U8_BE(reg1, 2))     ),
        (uint8_t)(aria_ct_lookup_u32(S2, GET_U8_BE(reg1, 3))     ));
    reg2 = rk->u[2] ^ MAKE_U32(
        (uint8_t)(aria_ct_lookup_u32(X1, GET_U8_BE(reg2, 0))     ),
        (uint8_t)(aria_ct_lookup_u32(X2, GET_U8_BE(reg2, 1)) >> 8),
        (uint8_t)(aria_ct_lookup_u32(S1, GET_U8_BE(reg2, 2))     ),
        (uint8_t)(aria_ct_lookup_u32(S2, GET_U8_BE(reg2, 3))     ));
    reg3 = rk->u[3] ^ MAKE_U32(
        (uint8_t)(aria_ct_lookup_u32(X1, GET_U8_BE(reg3, 0))     ),
        (uint8_t)(aria_ct_lookup_u32(X2, GET_U8_BE(reg3, 1)) >> 8),
        (uint8_t)(aria_ct_lookup_u32(S1, GET_U8_BE(reg3, 2))     ),
        (uint8_t)(aria_ct_lookup_u32(S2, GET_U8_BE(reg3, 3))     ));
 ...
 * Uses constant-time lookups to mitigate cache-timing attacks.
 */
static void sl1(ARIA_u128 *o, const ARIA_u128 *x, const ARIA_u128 *y)
{
    unsigned int i;
    for (i = 0; i < ARIA_BLOCK_SIZE; i += 4) {
        o->c[i    ] = aria_ct_lookup_u8(sb1, x->c[i    ] ^ y->c[i    ]);
        o->c[i + 1] = aria_ct_lookup_u8(sb2, x->c[i + 1] ^ y->c[i + 1]);
        o->c[i + 2] = aria_ct_lookup_u8(sb3, x->c[i + 2] ^ y->c[i + 2]);
        o->c[i + 3] = aria_ct_lookup_u8(sb4, x->c[i + 3] ^ y->c[i + 3]);
    }
}
...
static void sl2(ARIA_c128 o, const ARIA_u128 *x, const ARIA_u128 *y)
{
    unsigned int i;
    for (i = 0; i < ARIA_BLOCK_SIZE; i += 4) {
        o[i    ] = aria_ct_lookup_u8(sb3, x->c[i    ] ^ y->c[i    ]);
        o[i + 1] = aria_ct_lookup_u8(sb4, x->c[i + 1] ^ y->c[i + 1]);
        o[i + 2] = aria_ct_lookup_u8(sb1, x->c[i + 2] ^ y->c[i + 2]);
        o[i + 3] = aria_ct_lookup_u8(sb2, x->c[i + 3] ^ y->c[i + 3]);
    }
}
